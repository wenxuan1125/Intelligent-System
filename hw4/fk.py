...
def your_fk(robot, DH_params : dict, q : list or tuple or np.ndarray) -> np.ndarray:
    ...
    base_pos = robot._base_position             # tuple (x, y, z) = (-0.2, 0.13, 0.6)
    base_pose = list(base_pos) + [0, 0, 0, 1]   # [0, 0, 0, 1] -> quaternion, list [x, y, z, 0, 0, 0, 1] = [-0.2, 0.13, 0.6, 0, 0, 0, 1], 
    ...
    # A: transformation matrix which can change the robot form "base_pose" coordinate to "world" coordinate
    ...
    Ts = []
    for i, theta in enumerate(q):     # compute transformation mation form joint 1
        
        a = DH_params[i]['a']
        d = DH_params[i]['d']
        alpha = DH_params[i]['alpha']

        Trx = np.array([[1, 0, 0, 0], 
        ...
        Tx = np.array([[1, 0, 0, a], 
                        ...
                        [0, 0, 1, 0],
                        [0, 0, 0, 1]])
        Trz = np.array([[math.cos(theta), -math.sin(theta), 0, 0], 
    ...
    pose_7d = np.asarray(get_pose_from_matrix(A))   # shape = (7,), (pos_x, pos_y, pos_z, rot_x, rot_y, rot_z, rot_w)
    ...
    for i in range(7):
        T = Ts[i]
        axis = T[0:3, 2]
        r = pose_7d[0:3] - T[0:3, 3]    # end point - origin of joint i
        jacobian[0:3, i] = cross(axis, r)
        jacobian[3:6, i] = axis
...
def score_fk(robot, testcase_files : str, visualize : bool=False):
    ...
    for file_id, testcase_file in enumerate(testcase_files):
        ...
        for i in range(cases_num):
            ...
            # print('Your j: ', your_jacobian)
            # print('Ground j: ', np.asarray(jacobians[i]))
            # print('------------------------------')
