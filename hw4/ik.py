...
def your_ik(robot, new_pose : list or tuple or np.ndarray, 
    ...
    for _ in range(max_iters):
        # q: theta1-theta7, list
        tmp_pose_7d, jacobian = your_fk(robot, DH_params, tmp_q)
        tmp_pose_6d = np.asarray(pose_7d_to_6d(tmp_pose_7d))

        if(np.linalg.norm(tmp_pose_7d - np.asarray(new_pose)) < stop_thresh):
            break

        if(np.linalg.norm(tmp_pose_7d - np.asarray(new_pose)) > 0.1):  
            step = 0.06
        elif(np.linalg.norm(tmp_pose_7d - np.asarray(new_pose)) > 0.08):  
            step = 0.05
        elif(np.linalg.norm(tmp_pose_7d - np.asarray(new_pose)) > 0.05):
            step = 0.04
        elif(np.linalg.norm(tmp_pose_7d - np.asarray(new_pose)) > 0.01):
            step = 0.03
        elif(np.linalg.norm(tmp_pose_7d - np.asarray(new_pose)) > 0.003):
            step = 0.01
        else:
            step = 0.005

        # if(_ == 0):
        #     print(np.linalg.norm(tmp_pose_7d - np.asarray(new_pose)))
        # if(_ == max_iters - 1):
        #     print(np.linalg.norm(tmp_pose_7d - np.asarray(new_pose)))

        T_tmp = get_matrix_from_pose(tmp_pose_6d)
        T_new = get_matrix_from_pose(new_pose)
        T_delta = T_tmp @ scipy.linalg.inv(T_new)

        # tmp_q  =  tmp_q - step * jacobian.transpose() @ (tmp_pose_6d - np.asarray(pose_7d_to_6d(new_pose)))
        tmp_q  =  tmp_q - step * jacobian.transpose() @ (get_pose_from_matrix(T_delta, 6))


        for i, is_greater in enumerate(tmp_q > joint_limits[:, 1]):
            if(is_greater):
                
                tmp_q[i] = (joint_limits[i, 1] + joint_limits[i, 0])/2
                if(i-1>0):
                    tmp_q[i-1] = (joint_limits[i-1, 1] + joint_limits[i-1, 0])/2
                if(i+1<7):
                    tmp_q[i+1] = (joint_limits[i+1, 1] + joint_limits[i+1, 0])/2

        for i, is_smaller in enumerate(tmp_q < joint_limits[:, 0]):
            if(is_smaller):
...
def score_ik(robot, testcase_files : str, visualize : bool=False):
    ...
    for file_id, testcase_file in enumerate(testcase_files):
        ...
        for i in range(cases_num):
            ...
            # print('Your joint: ', your_joint_poses)
            # print('Pybullet: ', pybullet_ik(robot, poses[i]))
            # print('your pose: ', your_pose)
            # print('Ground: ', gt_pose)
            # print('Error: ', ik_error)
            # print('------------------------------')
            print(i, " ", ik_error > IK_ERROR_THRESH)
